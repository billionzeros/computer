...
Workflows are installable automation packages that create multiple independent routines, each handling a focused task. Routines coordinate through a shared SQLite database (not external connectors). Nothing runs until the user fully approves the setup through an iterative plan-review loop.

### Three Layers of Routine Instructions

Each routine has three layers that compose its runtime behavior:
...
### Independent Routines (No Orchestrator)

Each routine is small, focused, and cheap. They coordinate through the Google Sheet's status column:

| Routine | Reads Status | Writes Status | Schedule |
|---------|-------------|---------------|----------|
...
The sheet is the message bus. No inter-routine communication needed.

### Routine Lifecycle

```
Workflow Installed (no routines)
    ↓
...
activate_workflow tool called
    ↓
N routines created with:
  - workflowId (groups them)
  - workflowAgentKey (identifies which manifest entry)
  - Per-routine cron schedule
  - Per-routine prompt loaded at runtime
    ↓
Routines run independently on schedule
```

### Per-Routine Context Loading

When a routine runs, `buildWorkflowAgentContext(projectId, workflowId, agentKey)` loads:
- Only that routine's prompt file (not all prompts concatenated)
- Shared user config (merged defaults + user answers)
- That routine's scripts only
...
### RoutineMetadata (agent.json)

New fields:
- `workflowId?: string` — groups routines by workflow
- `workflowAgentKey?: string` — maps to manifest agent entry

### InstalledWorkflow (installed.json)

New field:
- `agentSessionIds?: string[]` — all routine session IDs (populated after activation)

### Protocol Messages

- `workflow_activate` (client → server) — triggers routine creation
- `workflow_activated` (server → client) — confirms routines created, returns routine list
...
- Creates N routines, sets workflowId + workflowAgentKey on each
- Sends `workflow_activated` to client with all created routines
...
- Integrated into RoutinesView with Flow/Routine tabs

### Uninstall

`WorkflowInstaller.uninstall()` finds all routines with matching `workflowId` and deletes them all.

## Files Changed

### Protocol (3 files)
- `packages/protocol/src/projects.ts` — `workflowAgentKey` on RoutineMetadata
...
### Server (3 files)
- `packages/agent-server/src/workflows/workflow-installer.ts` — install (no routines) + activateWorkflow (N routines) + uninstall (all routines)
- `packages/agent-server/src/workflows/workflow-context.ts` — per-routine context loading
...
- `packages/desktop/src/lib/store/handlers/projectHandler.ts` — workflow_activated + dedup + navigation fixes (routine list update)
- `packages/desktop/src/components/routines/RoutinesView.tsx` — Flow/Routine tabs
- `packages/desktop/src/components/routines/RoutineListView.tsx` — project name lookup, two-line layout
...
1. **Independent routines are genuinely better.** Each routine has a tiny, focused prompt. Runs are cheap. Users can add/remove/customize individual routines. Debugging is per-routine. This is the right architecture.
...
1. **Bootstrap depends on model following a precise 4-phase prompt.** The model might skip phases, call `activate_workflow` prematurely, forget to call it, or jumble the order. The bootstrap prompt is well-structured but there's no programmatic enforcement. If the model goes off-script, the user gets a broken experience.
   ...
   **Mitigations (implemented):**
   ...
   - `activate_workflow` tool only available to human conversations (not routine sessions)
...
3. **Routine scheduling relies on staggered cron offsets.** Lead Scanner at :00, Lead Scorer at :30, Outreach Writer at :00 odd hours. If the scanner run takes longer than 30 minutes (slow Gmail API, many leads), the scorer might run before the scanner finishes — picking up stale data or missing new leads. There's no dependency graph enforcement.
...
4. **Per-routine context loading loses cross-routine awareness.** The lead-scorer routine doesn't know what the outreach-writer does, and vice versa. If they need to coordinate beyond the status column (e.g., scorer needs to know the outreach threshold to prioritize scoring), that context must be in the shared config or the scorer's own prompt.

   **Current state:** The shared user config (ICP, threshold, etc.) is loaded for all routines, which covers most cross-routine data needs. But if workflows get more complex, this could become a limitation.
...
2. **The `WorkflowAgentRef.role` field is now redundant.** All 3 routines are `role: "main"`. The `"sub"` role was for the old concatenated approach. With independent routines, every routine is "main". The field exists for backward compatibility but adds confusion.

   **Recommendation:** Consider deprecating `role: "sub"` in favor of always independent routines. If a routine truly shouldn't have its own schedule, just omit the `schedule` field.

### What Won't Work As Expected

1. **~~The `activate_workflow` tool is available to ALL project-scoped sessions.~~** FIXED: Tool is now only provided to human conversations (`!isAgent`), not routine runs. Server-side `activateWorkflow()` also throws if already activated.
...
1. **No way for users to add/remove routines from the UI yet.** The architecture supports it (each routine is independent), but there's no UI to add a 4th routine or remove the outreach writer.

2. **No dependency graph between routines.** Routines are staggered by cron offset, but there's no guarantee scanner finishes before scorer starts. The status column prevents processing incomplete data, but a proper dependency system (routine B waits for routine A to complete) would be more robust for complex workflows.
